...
// conditional Rendering
    ...
    return listOfRestaurents?.length === 0 ? (<Shimmer/>) : (
      ...
      <div className="search flex justify-center m-2 p-5 content-center ">
          <input className="p-2 w-2/5 border border-solid rounded-md border-green-400
           " 
          type="text" placeholder="Search a restaurant you want..." 
           ...
           />
          <button type="submit" 
          className="search-btn p-2 text-yellow-600/100 font-thin text-3xl hover:cursor-pointer"
          onClick={ () => {
        ...
        <input className="p-2 w-3/5 border border-solid rounded-md border-green-400
           " 
          type="text"  
            value={logedInUser}
            onChange={(e) => {
